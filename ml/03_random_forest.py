...
if __name__ == "__main__":
    ...
    spark = (
        ...
        .appName(f"RF with Lag {month_u}")
        # base URI; DB/collection are set per-write
    ...
    # 11) Save ML Predictions in Mongo
    (pred.select(
            "zone_id",
        ...
        .withColumn("model", lit("rf"))     # <-- tag as Random Forest (rf)
        .write
        .format("mongodb")
        .mode("append")
        .option("database", "timeseries_demo")
        .option("collection", "readings_pred_rf")
        .save())
